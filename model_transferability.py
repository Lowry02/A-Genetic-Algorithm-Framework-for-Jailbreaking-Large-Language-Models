...
# loading the dataset
suffixes = pd.read_csv("/log/detokenized_8B.csv")
...
with open(file_name, mode='w', newline='') as file:
    ...
    for question in tqdm(questions.values):
        ...
        for i, suffix in enumerate(suffixes.values):
            type, N, run, _, string = suffix #"type", "N", "run", "suffix", "string"
            adv_prompt = PROMPT.format(target_answer=target, goal=question) + string
